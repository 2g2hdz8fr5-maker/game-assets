...
async function download() {
  ...
  page.on('response', async (resp) => {
    ...
    if (url.includes('download') || url.includes('.zip') || contentType.includes('zip') || 
        contentType.includes('octet-stream') || url.includes('/f/d') || url.includes('resc-')) {
      console.log('  [INTERCEPT] ' + url + ' (' + contentType + ')');
  ...
  for (const pack of PACKS) {
    ...
    try {
      ...
      console.log('  Captured URLs: ' + JSON.stringify(downloadUrls));
      
      // Wait and check for more responses
